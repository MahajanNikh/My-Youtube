...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    {
      $set: {
        fullName, // we can also write it like fullName: fullName
        email,
      },
    },
    { new: true }
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  // User.find({ username }); we can also do this like..
...
//Ageregation Pipeline (Complex Code)
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      //  In Aggeregate Pipline code directly passes to mongodb and (req.user._id= string) in MongoDb
      $match: {
